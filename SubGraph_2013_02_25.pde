...
boolean PAUSE=false;
int FRAMERATE=60;
int REPULSION = 500000; // 5000 to 500000;
float ATTRACTION = 3;   // 0.5 to 3.0;
...
background (0);
frameRate(FRAMERATE);
String[] lines = loadStrings("order("+K+").txt");//("tree_experiment_ID.txt");
String[] lines1 = loadStrings("pname("+K+").txt");//("tree_experiment_name.txt");
...
allNodes.get(0).mX = width/2;
allNodes.get(0).mX = height/2;
//for (int k=start; k<=end; k++)
  //for
...
curr.mScore = impToSize(imp[1], MAX, MIN);
}
...
temp1.neighbors.add(new Neighbor(temp0, num[2]));
}
//println(start, end);
