...
public class SubGraph_2013_02_25 extends PApplet {
...
float ATTRACTION = 2.2f;   // 0.5 to 3.0;
...
public float edgeBrightness(float connectionLevel)
{
  float scale = (999.0f-THRESHOLD)/160;
float s = (connectionLevel - THRESHOLD)/scale+10;
return s;
}

public float impToSize(float imp, float scaleMax, float scaleMin)
{
...
public boolean mouseOverCircle(float x, float y, float diameter) 
{  
  return (dist(mouseX, mouseY, x, y) <= diameter*0.5f);
}
...
int[] num0 = PApplet.parseInt(split(lines[lines.length-1],' '));
int[] num1 = PApplet.parseInt(split(lines[0],' '));
//println(imp[0],imp[1]);
int start = num1[0];
int end = num0[0];
float[] num2 = PApplet.parseFloat(split(lines1[lines1.length-1], ' '));
float[] num3 = PApplet.parseFloat(split(lines1[0], ' '));
println(num2[1]);
float MIN = num2[1];
float MAX = num3[1];
...
float[] imp = PApplet.parseFloat(split(lines1[k-1],' '));
curr.mName = name[0];
...
{
int[] num = PApplet.parseInt(split(lines[i],' '));
if (num[1] > end)
  {
    end = num[1];
...
curr.mForce.add(new PVector(0,-300));
curr.mX += 0.06f*(curr.mForce.x);
curr.mY += 0.06f*(curr.mForce.y);
...
if (mouseOverCircle(temp.mX, temp.mY, temp.mScore))
  {
    ...
    //pushMatrix();
    fill(0,255,255);
    text(temp.mName, temp.mX+0.5f*temp.mScore, temp.mY+0.5f*temp.mScore);
    fill(0, 0, 255);
    //popMatrix();
  }
else
...
line(temp.mX, temp.mY, temp2.mX, temp2.mY);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "SubGraph_2013_02_25" };
    if (passedArgs != null) {
